...
app.post('/book/delete/:id', (req, res) => {
  ...
  const bookDelete = `DELETE FROM books WHERE ?? = ?`
  const data = ['id', id]

  pool.query(bookDelete, data, (err) => {
    if(err) {
...
app.get('/book/edit/:id', (req, res) => {
  ...
  const searchBook = `SELECT * FROM books WHERE ?? = ?`
  const data = ['id', id]
  pool.query(searchBook, data, (err, data) => {
    if(err) {
      return console.log(err)
    }
...
app.get('/book/:id', (req, res) => {
  ...
  const selectedBook = `SELECT * FROM books WHERE ?? = ?`
  const data = ['id', id]

  pool.query(selectedBook, data, (err, data) => {
    if(err) {
      return console.log(err)
    }
...
app.post('/book/register', (req, res) => {
  ...
  const registerBook = `INSERT INTO books (??, ??) VALUES(?, ?)`
  const data = ['title', 'pageqtd', title, pageqtd]
  pool.query(registerBook, data, (err) => {
    if(err) {
      return console.log(err)
    }
